...
>
> Parte 3 pronta! Aqui está o resumo do que ficou capturado:
CIF de Inmon — os quatro princípios core (top-down, orientado por assunto, integrado, time-variant, non-volatile) cada um com a sua secção própria, com ligações ao Databricks onde são relevantes.
Arquitectura CIF completa — diagrama ASCII com todos os componentes: fontes → ETL → ODS → EDW → Data Marts → ferramentas de acesso, incluindo o Repositório de Metadados.
ETL detalhado — as três fases com o que o Transform envolve, mais nota sobre o equivalente no Databricks (DLT/Lakeflow).
Data Marts — tipos dependente vs. independente, benefícios e integração com o EDW.
Tabela de mapeamento Inmon → Medallion — a secção que eu acho mais útil para o exame: Bronze = fontes raw, Silver = ODS/EDW, Gold = Data Marts, Unity Catalog = Repositório de Metadados.
